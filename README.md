...
| <a name="input_self_service_saml_provider_arn"></a> [self\_service\_saml\_provider\_arn](#input\_self\_service\_saml\_provider\_arn) | The ARN of the IAM SAML identity provider for the Client VPN self-service portal. Defaults to saml\_provider\_arn when the portal is enabled and this value is not set. | `string` | `null` | no |
